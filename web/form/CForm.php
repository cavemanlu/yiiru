...
/**
 * Класс CForm представляет объект формы, содержащий спецификацию элементов
 * ввода данных.
 * 
 * Основная цель включения абстракции объектов форм - это улучшение возможности
 * повторного использования форм. В частности, можно разделить форму на две
 * части - та, которая определяет каждый отдельный элемент ввода данных, и та,
 * которая определяет внешний вид элементов ввода. Объект класса CForm
 * представляет первую часть. It relies on the rendering process to
...
{
	/**
	 * @var string заголовок данной формы. По умолчанию, если установлен, то
	 * форма может быть обрамлена в набор (fieldset), при этом заголовок будет
	 * использован в качестве легенды (legend) набора. По умолчанию - null
	 */
	public $title;
	/**
	 * @var string описание данной формы
	 */
	public $description;
	/**
	 * @var string метод отправки формы. По умолчанию - 'post'. Данное свойство
	 * игнорируется, если данная форма является подформой
	 */
	public $method='post';
	/**
	 * @var mixed URL-адрес действия формы (см. {@link CHtml::normalizeUrl} для
	 * детального описания параметра). ПО умолчанию - пустая строка, что
	 * означает использование URLадреса текущего запроса. Свойство игнорируется,
	 * если данная форма является подформой
	 */
	public $action='';
	/**
	 * @var string имя класса, представляющего собой элемент ввода формы. По
	 * умолчанию - 'CFormInputElement'
	 */
	public $inputElementClass='CFormInputElement';
	/**
	 * @var string имя класса, представляющего собой кнопку формы. По
	 * умолчанию - 'CFormButtonElement'
	 */
