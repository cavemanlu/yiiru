...
/**
 * Класс CActiveFinder реализует "жадную" и "ленивую" загрузки связанных
 * AR-объектов.
 *
 * При использовании "жадной" загрузки данный класс предоставляет тот же набор
...
class CActiveFinder extends CComponent
{
	...
	/**
	 * Конструктор.
	 * Дерево соединений строится на основе определенных отношений между
	 * AR-объектами
	 * @param CActiveRecord $model модель, инициализирующая процесс активного
	 * поиска
	 * @param mixed $with имена отношений, по которым проводить соединение
	 */
	...
	/**
	 * Не вызывайте данный метод. Требуется для внутреннего использования для
	 * выполнения реляционного запроса на основе переданного критерия
	 * @param CDbCriteria $criteria критерий запроса
	 * @param boolean $all возвращать ли все записи
	...
	/**
	 * Требуется для внутреннего использования
	...
	/**
	 * Находит связанные объекты определенного AR-объекта. Метод используется
	 * для внутреннего вызова {@link CActiveRecord} объетом для поддержки
	 * "ленивой" загрузки
	 * @param CActiveRecord $baseRecord базовая запись, связанные объекты
	 * которой подгружаются
	 */
	...
	/**
	 * Строит дерево соединений, представляющее связи, включенные в запрос
	 * @param CJoinElement $parent родительский узел дерева
	 * @param mixed $with имена связанных объектов родительского узла дерева
	 * @param array $options дополнительные параметры запроса для связи
	 */
...
/**
 * Класс CJoinElement представляет узел дерева соединений, создаваемого
...
{
	/**
	 * @var integer уникальный идентификатор данного узла дерева
	 */
	public $id;
	/**
	 * @var CActiveRelation связь, представленная данным узлом дерева
	 */
	public $relation;
	/**
	 * @var CActiveRelation основная связь
	 */
	public $master;
	/**
	 * @var CActiveRelation подчиненная связь
	 */
	public $slave;
	/**
	 * @var CActiveRecord модель, ассоциированная с данным узлом дерева
	 */
	public $model;
	/**
	 * @var array список AR-объектов, найденных запросами. Индексирован по
	 * значениям первичного ключа
	 */
	public $records=array();
	/**
	 * @var array список дочерних элементов соединения
	 */
	...
	public $stats=array();
	/**
	 * @var string псевдоним таблицы для данного элемента соединения
	 */
	public $tableAlias;
	/**
	 * @var string экранированный псевдоним таблицы для данного элемента
	 */
	...
	/**
	 * Конструктор
	 * @param CActiveFinder $finder объект поиска
	 * @param mixed $relation отношение (если третий параметр не null) или
	 * модель (если третий параметр - null), ассоциированные с данным узлом
	 * дерева связей
	 * @param CJoinElement $parent родительский узел дерева связей
	 * @param integer $id идентификатор данного узла дерева связей, уникальный
	 * среди всех узлов дерева
	 */
	...
	/**
	 * Удаляет ссылки на элементы-потомки и объект поиска во избежание
	 * цикличных ссылок. Для внутреннего использования
	 */
	...
	/**
	 * Выполняет рекурсивный поиск с учетом критерия
	 * @param CDbCriteria $criteria критерий запроса
	 */
	...
	/**
	 * Применяет условие "ленивой" загрузки
	 * @param CJoinQuery $query представляет SQL-выражение JOIN
	 * @param CActiveRecord $record AR-объект, связанный объект которого должен быть получен
	 */
	...
	/**
	 * Выполняет "жадную" загрузку для набора базовых подготовленных записей
	 * @param mixed $baseRecords доступные базовые записи
	 */
	...
	/**
	 * Количество первичных записей, возвращенных запросом соединения
	 * @param CDbCriteria $criteria критерий запроса
	...
	/**
	 * Создает запрос соединения со всеми узлами-потомками типов HAS_ONE и
	 * BELONGS_TO
	 * @param CJoinQuery $query создаваемый запрос
	 */
	...
	/**
	 * Выполняет запрос соединения и создает AR-объекты с данными запроса
	 * @param CJoinQuery $query выполняемый запрос
	 */
	...
	/**
	 * Создает AR-объект со свойствами, заполнеными данными запроса
	 * @param CJoinQuery $query выполняемый запрос
	 * @param array $row строка данных
	 * @return CActiveRecord заполненный данными AR-объект
	 */
	...
	/**
	 * @return string имя таблицы и псевдонима таблицы (если существует). Может
	 * использоваться непосредственно в SQL-запросе без экранирования
	 */
	...
	/**
	 * Генерирует список выбираемых столбцов. Столбцы будут заменены на их
	 * псевдонимы, а первичные ключи - добавлены к выборке, если они не
	 * определены
	 * @param mixed $select выбираемые столбцы. По умолчанию - '*', т.е.,
	 * выбираются все столбцы
	 * @return string строка выбранных столбцов
	 */
	...
	/**
	 * @return string выборка первичного ключа
	 */
	...
	/**
	 * @return string условие, определяющее только строки с выбранными значениями первичного ключа
	 */
	...
	/**
	 * @return string префикс столбца для неоднозначных ссылочных столбцов
	 */
	...
	/**
	 * @return string join-выражение (данный узел соединяется с его родительским узлом)
	 */
	...
	/**
	 * Генерирует выражение соединения для связи один-ко-многим.
	 * Работает для типов связей HAS_ONE, HAS_MANY и BELONGS_TO
	 * @param CJoinElement $fke элемент соединения, содержащий внешние ключи
	 * @param array $fks внешние ключи
	 * @param CJoinElement $pke элемент соединения, содержащий первичные ключи
	 * @param CJoinElement $parent родительский элемент соединения
	 * @return string выражение соединения
	 * @throws CDbException вызывается, если внешний ключ неверен
	 */
	...
	/**
	 * Генерирует выражение соединения для связи многие-ко-многим
	 * @param CDbTableSchema $joinTable таблица соединений
	 * @param array $fks внешние ключи
	 * @param CJoinElement $parent родительский элемент соединения
	 * @return string выражение соединения
	 * @throws CDbException вызывается, если внешний ключ неверен
...
{
	/**
	 * @var array спсок выбираемых столбцов
	 */
	...
	/**
	 * @var array список выражений соединения
	 */
	public $joins=array();
	/**
	 * @var array список выражений WHERE
	 */
	public $conditions=array();
	/**
	 * @var array список выражений ORDER BY
	 */
	public $orders=array();
	/**
	 * @var array список выражений GROUP BY
	 */
	public $groups=array();
	/**
	 * @var array список выражений HAVING
	 */
	public $havings=array();
	/**
	 * @var integer лимит строк
	 */
	public $limit=-1;
	/**
	 * @var integer смещение строк
	 */
	public $offset=-1;
	/**
	 * @var array список параметров запроса
	 */
	public $params=array();
	/**
	 * @var array список идентификаторов соединяемых элементов (id=>true)
	 */
	public $elements=array();

	/**
	 * Конструктор
	 * @param CJoinElement $joinElement Корень дерева соединений
	 * @param CDbCriteria $criteria критерий запроса
	 */
	...
	/**
	 * Соединяет дынный элемент с другим
	 * @param CJoinElement $element присоединяемый элемент
	 */
	...
	/**
	 * Создает SQL-выражение
	 * @param CDbCommandBuilder $builder построитель команд
	 * @return CDbCommand экземпляр команды БД, представляющей SQL-выражение
	 */
...
/**
 * Класс CStatElement представляет соединяемый элемент статистических запросов
...
{
	/**
	 * @var CActiveRelation отношение, представленное данным узлом дерева
	 */
	public $relation;
	...
	/**
	 * Конструктор
	 * @param CActiveFinder $finder объект поиска (finder)
	 * @param CStatRelation $relation статистическое отношение (STAT relation)
	 * @param CJoinElement $parent элемент соединения, которому принадлежит
	 * данный статистический элемент
	 */
	...
	/**
	 * Выполняет статистический запрос
	 */
	...
	/*
	 * @param string $joinTableName имя присоединяемой таблицы
